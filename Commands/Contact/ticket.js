...
export const command = {
	...
	run: async (bot, message, args, config) => {
		...
		if (config.tfooter?.trim()) {
			embed.setFooter({ text: config.tfooter, iconURL: message.guild.iconURL({ dynamic: true }) ?? undefined });
		}

		await message.channel.send({ embeds: [embed], components: [new ActionRowBuilder().addComponents(ticket)] });
		await message.reply(`Le système de tickets utilise maintenant la catégorie ${category}.`);
	},
};
